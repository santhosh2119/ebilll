...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form")) {
  $updateSQL = sprintf("UPDATE gym SET ownerid=%s, invoiceno=%s, cname=%s, mobile=%s, course=%s, joindate=%s, enddate=%s, tamount=%s, paidamount=%s, `date`=%s, `time`=%s WHERE sno=%s",
...
mysql_select_db($database_ebils, $ebils);
$query_Recordset1 = sprintf("SELECT * FROM gym WHERE sno = %s", GetSQLValueString($colname_Recordset1, "int"));
...
<div class="col-md-6">
                                        ...
                                        <input class="form-control" placeholder="Shoulders, Arms, 6 packs"name="course" value="<?php echo $row_Recordset1['course']; ?>" required/>
                                    </div> 
