...
$sql = "SELECT * FROM busers where mobile =  '$mobile_no' ";
...
if ($result->num_rows === 1) {
        ...
        if ($result == true) {
          ...
          mysqli_query($con, "INSERT INTO forget (mobile,otp_hash) VALUES( '" . $mobile_no . "', '" . $rand_opt . "')"); 
...
if (isset($_POST['otpverify'])) {
	...
	if($password != $cpassword) {
		...
		if (!$error) {
$sql = "SELECT * FROM forget where mobile ='$mobile_no' AND otp_hash='$opt' ";
$result = $con->query($sql);
	
	
if ($result->num_rows === 1) {
    // output data of each row
 mysqli_query($con, "UPDATE `busers` SET `password`= '$pass' WHERE mobile = '$mobile_no'");
	header("Location: changedlogin.php"); 
}
	else{
			$error = true;
...
<form method="POST" name ="verify" action="forgetverify.php">
